...
**目标引擎：团结引擎 1.10.0 · 文档版本：1.11.2 · 日期：2026-09-06 · 基于 v1.4 玩法与 v1.5 美术增量开发**
...
> **当前状态：** 《剑会变长》v1.11.2 已完成连续关卡解锁：初始只开放第 1 关，通关逐关开放并持久化；选关页可确认解锁全部。第一关从关卡选择进入时会显示操作与目标教程，包含移动、转剑、果实生长、鼠标左键确认和“Space 等待，经过一拍”，关闭前不接收回合输入。平移时剑砍果会回弹，身体拾果则停在目标果格后进入成长。弩手按“预备、射击、冷却”三拍循环行动。标题页提供退出游戏与“by 海蛋”作者署名。新增必须连续完成的数字多生长果与第 11 关多果推箱挑战。箱子透明边界已裁切，箱子 / 机关可见尺寸为 0.85 / 0.92 格。运行时标题与构建产品名均为《剑会变长》。Windows 64 位交付包已生成；编辑器 P1、作者指定形状复核、完整解谜仍未完成。
...
Windows 64 位构建输出统一放在工程根目录 `Builds/Windows/`，可执行文件命名为 `剑会变长.exe`。发布时必须一并保留同级的 `*_Data`、`UnityPlayer.dll` 和运行时 DLL；它们共同构成可运行包，不能只复制 `.exe`。字体许可证已随包放入 `Builds/Windows/Licenses/NotoSansSC-OFL.txt`。构建报告、产物大小和实际构建时间登记在本节末尾。

构建日期：2026-09-06；目标：Windows 64 位；入口场景：`Assets/_Game/Scenes/Game.scene`；首次构建报告：`Succeeded`，错误 0、警告 2、耗时 36.18 秒、BuildReport 汇总大小 124,974,385 bytes（约 119.2 MiB）。v1.11.2 教程改动后已重新构建：`Succeeded`，错误 0、警告 2、耗时 10.22 秒、BuildReport 汇总大小 124,974,673 bytes。加入字体许可证后，交付目录内 170 个文件合计约 119.3 MiB。两次构建的警告均来自 URP 后处理的 `FinalPost` / `Scaling Setup` 着色器循环展开，不影响成功产物。
